...
void Menu::menuUsuarios(Manager& manager, UiConsole& ui) {
	auto agregarUsuario = [&]() -> void {
		...
		if (!manager.getPrivilegios()) {
			std::cout << UiConsole::ROJO << "No tiene privilegios para realizar esta acci�n." << UiConsole::RESET << std::endl;
			ui.pausa();
			return;
		}
	...
	auto eliminarUsuario = [&]() -> void {
		...
		do {
			...
			if (posicionUsuario < 0) {
				...
				std::cout << UiConsole::ROJO << "�Est� seguro que desea eliminar el usuario " << UiConsole::BOLD << UiConsole::VERDE << "'" << nombreUsuario << "'" << UiConsole::ROJO << "?" << UiConsole::RESET << "(Y/N)" << std::endl;
	...
	auto modificarContraseniaRoot = [&]() -> void {
		if (!manager.getPrivilegios()) {
			std::cout << UiConsole::ROJO << "No tiene privilegios para modificar la contrase�a root." << UiConsole::RESET << std::endl;
			ui.pausa();
			return;
		}
